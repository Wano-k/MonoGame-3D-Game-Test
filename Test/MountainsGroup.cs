...
namespace RPG_Paper_Maker
{
    ...
    class MountainsGroup
    {
        ...
        public void CreatePortion(GraphicsDevice device, int id)
        {
            ...
            {
                List<VertexPositionTexture> vertexPositionTextures = CreateTex(Game1.TexReliefs[id], entry.Key, entry.Value);
        ...
        public void Draw(GraphicsDevice device, AlphaTestEffect effect, int id)
        {
            if (VB != null)
            {
                if (!Game1.TexReliefs.ContainsKey(id)) effect.Texture = Game1.TexNone;
                else effect.Texture = Game1.TexReliefs[id];
